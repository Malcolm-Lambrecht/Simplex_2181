...
namespace Simplex
{
	...
	class MyOctant
	{
		...
		bool m_bDisplay = true;//bool on weather or not the octtree is displayed
	public:
		void SetMyDimensions();
		void setEntityList(std::vector<uint> n);
		/*
		...
		OUTPUT: class object
		*/
		MyOctant(std::vector<uint> list, uint a_nMaxLevel = 2, uint a_nIdealEntityCount = 5);
		/*
		USAGE: Constructor
		ARGUMENTS:
