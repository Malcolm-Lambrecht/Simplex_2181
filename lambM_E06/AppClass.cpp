...
void Application::Display(void)
{
	...
	static float totalTime = 25.0f; //the total time to complete the path
	float timeBetween = totalTime / m_stopsList.size(); //time between each set of points
	float ratioComplete = fmod(fTimer, timeBetween);
	ratioComplete = ratioComplete / timeBetween; // ratio from 0-1 of time taken between current points
	//normalize to between 0 and 1

	//calculate which indices of the stops vector it should be lerping between
	int index = floor(fTimer / timeBetween);
	
	//std::cout << ratioComplete << std::endl;
	//std::cout << index << std::endl;

	if (index + 1 < m_stopsList.size()) {
		v3CurrentPos = glm::lerp(m_stopsList[index], m_stopsList[index + 1], ratioComplete);
	}
	else {
		fTimer = 0;
		index = 0;
